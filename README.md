...
| 子供の割り当てと警告表示。**ワンクリックでチーム名・URL・PINコードを含む共有メッセージをコピーし、LINEで簡単に共有できます。** | 完成した配車をメンバー全員で共有。誰がどの車に乗るか一目瞭然です。 |

<br/>

## 追加機能（ユーザーフィードバックを元に）

実際に使っていただいたユーザーのフィードバックをもとに機能を追加しました。

| 子供の参加可否 | 回答期限の設定 |
| ---- | ---- |
| ![子供の参加可否](./public/screenshots/child-availability.png) | ![回答期限](./public/screenshots/deadline.png) |
| 子供ごとに参加・不参加を入力できます。欠席の場合は配車割り当てに表示されません。 | 回答期限を設定できます。共有メッセージに「〇月〇日までにご回答をお願いします」が自動で追加されます。 |

| コメント機能 | 学年表示・学年順ソート |
| ---- | ---- |
| ![コメント機能](./public/screenshots/comment.png) | ![学年順ソート](./public/screenshots/grade-sort.png) |
| 配車ごとにコメントを残せる機能。管理者、メンバーで共有できアプリ内では解決できないことをコメントとして残せます。 | メンバー登録時に学年を選択でき、乗車員を確認する画面では自動で学年順に並び替えられます。|

| 参加者・欠席者一覧 | 引率可否機能 |
| ---- | ---- |
| ![参加者一覧](./public/screenshots/attendance-list.png) | ![引率可否](./public/screenshots/escort-input.png) |
| 子供ごとに参加・不参加を管理。参加者一覧・欠席者一覧で確認できます。 | 保護者が引率できるかどうかを入力できます。（車は出せないけど、誰かの車に乗って引率する場合など） |

| 保護者複数登録 | |
| ---- | ---- |
| ![保護者複数登録](./public/screenshots/multi-guardian.png) | |
| お父さん・お母さんなど、複数の保護者を登録できます。 | |
...
- **共有メッセージ一括コピー**: ボタン1つでチーム名・URL・PINコードを含む共有用テキストをコピー。LINEグループに貼り付けるだけでメンバー全員に配車情報を共有できます
- **PINコードを忘れない**: 共有メッセージにPINコードが含まれているので、メンバーがPINを忘れてもLINEのトーク履歴から確認可能
- **迷わない導線**: PIN入力後、配車一覧ページに自動遷移。メンバーは迷わず配車可否を入力できます
...
- [x] 配車の割当を行った場合は配車一覧ページでわかるようにしたい（ステータス表示）
...
- [x] メンバー登録の時に父と母を別々に登録できるようにしたい（複数の保護者対応）
- [ ] 管理者の変更機能
- [ ] アカウント削除機能
...
バグ報告・機能要望・ご質問などがございましたら、お気軽にお問い合わせください。

- **Email**: [carpool.app.2026@gmail.com](mailto:carpool.app.2026@gmail.com)
