...
const steps = ["家族を登録", "日程を作成", "回答を集める", "配車案を作る"];
...
export function GettingStartedContent({ teamId, data }: { teamId: string; data: GettingStartedResponse }) {
  ...
  useEffect(() => {
    if (data.hasTriedAutoAssign || !ref.current || typeof IntersectionObserver === "undefined") return;
  ...
  }, [teamId, action.step, data.hasTriedAutoAssign]);

  if (data.hasTriedAutoAssign) return null;
  return (
    <section ref={ref} aria-label="はじめての配車" className="mb-6 rounded-2xl border border-teal-200 bg-teal-50/70 p-5 sm:p-6">
      <p className="text-sm font-semibold text-teal-700">はじめての配車</p>
      <ol className="my-4 grid grid-cols-2 gap-3 sm:grid-cols-4" aria-label="配車案を作るまでの手順">
        {steps.map((label, index) => (
          <li key={label} aria-current={index === action.step ? "step" : undefined} className={`flex items-center gap-2 text-xs ${index === action.step ? "font-bold text-teal-950" : "text-gray-600"}`}>
            <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${index <= action.step ? "bg-teal-700 text-white" : "bg-white text-gray-500"}`}>
              {index < action.step ? <><Check size={14} aria-hidden="true" /><span className="sr-only">完了</span></> : index + 1}
            </span>{label}
          </li>
        ))}
      </ol>
...
export default function GettingStartedCard({ teamId, afterRegistration = false }: { teamId: string; afterRegistration?: boolean }) {
